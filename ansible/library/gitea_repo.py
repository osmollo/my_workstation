...
api_url = "https://gitea.com/api/v4"
...
def gitea_get_user_id(data=None):
    headers = {
        "PRIVATE-TOKEN": "{}" . format(data['gitea_auth_key'])
    }
...
def gitea_get_project_id(data=None):
    headers = {
        "PRIVATE-TOKEN": "{}" . format(data['gitea_auth_key'])
    }
    url = "{}/users/{}/projects?pagination=keyset&per_page=99&order_by=id" . format(api_url, gitea_get_user_id(data))
    result = requests.get(url, headers=headers)
...
def gitea_repo_absent(data=None):
    ...
    url = "{}/projects/{}" . format(api_url, gitea_get_project_id(data))
